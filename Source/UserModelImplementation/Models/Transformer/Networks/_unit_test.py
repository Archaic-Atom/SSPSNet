...
class UnitTest(object):
    ...
    def _multi_gpu_test(self) -> None:
        ...
        left_img = torch.rand(1, 3, 448, 224).to(device)
        right_img = torch.rand(1, 3, 448, 224).to(device)

        # model = FSNet()
        model = StereoT(3, 0, 196, 'dinov2', False)
    ...
    def _pre_trained_multi_gpu_test(self) -> None:
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        left_img = torch.rand(1, 3, 448, 224).to(device)
        right_img = torch.rand(1, 3, 448, 224).to(device)
    ...
    def exec(self, args: object) -> None:
        self._pre_trained_multi_gpu_test()
