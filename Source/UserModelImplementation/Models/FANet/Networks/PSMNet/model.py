...
class MatchingModule(nn.Module):
    def __init__(self, inplanes: int) -> object:
        super().__init__()
        self.dres0 = nn.Sequential(convbn_3d(inplanes, 196, 3, 1, 1),
                                   nn.ReLU(inplace=True),
                                   convbn_3d(196, 128, 3, 1, 1),
                                   nn.ReLU(inplace=True))
        self.dres1 = nn.Sequential(convbn_3d(128, 128, 3, 1, 1),
                                   nn.ReLU(inplace=True),
                                   convbn_3d(128, 128, 3, 1, 1))
        self.dres2 = hourglass(128)
        self.dres3 = hourglass(128)
        self.dres4 = hourglass(128)
...
class PSMNet(nn.Module):
    def __init__(self, maxdisp: int, mask=False) -> object:
        super().__init__()
    ...
    def forward(self, left: torch.Tensor, right: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            if self.mask:
                return pred1, pred2, pred3, recovery_img
