...
class FANet(nn.Module):
    ...
    def __init__(self, in_channles: int, start_disp: int, disp_num: int,
        ...
        self.matching_module = build_matching_module(776)
    ...
    def _get_decode_module(self) -> nn.Module:
        deonv_1 = nn.Sequential(nn.Conv3d(384, 64, kernel_size=3, padding=1, stride=1, bias=False),
                                nn.BatchNorm3d(64),
    ...
    def _matching_module_proc(self, cost: torch.Tensor) -> torch.Tensor:
        ...
        cost = self.matching_module(cost)[self.ID_FEAT]
    ...
    def _mask_fine_tune_proc(self, left_img: torch.Tensor, right_img: torch.Tensor,
        ...
        left_img = self.feature_extraction(left_img)[self.ID_FEAT]
        right_img = self.feature_extraction(right_img)[self.ID_FEAT]
    ...
    def _mask_pre_train_proc(self, left_img: torch.Tensor,
                             right_img: torch.Tensor) -> torch.Tensor:
        return [self.feature_extraction(left_img),
                self.feature_extraction(right_img)]
