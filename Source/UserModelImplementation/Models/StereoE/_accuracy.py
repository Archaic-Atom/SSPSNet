...
class Accuracy(object):
    ...
    def matching_accuracy(self, output_data: list, disp_label: torch.Tensor,
                          id_error_px: int = 1, invalid_value: int = 0) -> list:
        """Return accuracy/MAE pairs for refined + coarse predictions."""
        assert len(output_data) > 0
        match_out = output_data[0]
        final_disp = match_out["disp_full"]
        coarse_disp = match_out["disp_coarse"]

        res = []
        for disp in (final_disp, coarse_disp):
