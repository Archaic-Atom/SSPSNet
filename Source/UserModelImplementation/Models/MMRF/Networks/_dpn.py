...
try:
    from ._nmp import MLP, Propagation, PropagationLayer
except ImportError:
    from _nmp import MLP, Propagation, PropagationLayer
...
class DPN(nn.Module):
    ...
    def __init__(self, cost_group=4, num_proposals=4, feat_dim=256, context_dim=64, num_prop_layers=5,
                 prop_embed_dim=128, mlp_ratio=4, split_size=1, prop_n_heads=4, activation="gelu",
    ...
    def forward(self, cost_volume, fmap1):
        """
        ...
        bs, group, nd, ht, wd = cost_volume.shape
        ...
        out = torch.nn.functional.max_pool1d(
            prob.unsqueeze(-2), kernel_size=3, stride=1, padding=1).squeeze(-2)
        ...
        context = self.proj(fmap1)  # visual context is used in affinity computation
