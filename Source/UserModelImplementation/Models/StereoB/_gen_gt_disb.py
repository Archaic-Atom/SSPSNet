...
def LaplaceDisp2Prob(gt: torch.Tensor, maxdisp=192):
    N, H, W = gt.shape
    b = 0.8
...
def adaptive_multi_modal_cross_entropy_loss(x, disp, mask, maxdisp,
                                            m=1, n=9, top_k=9, epsilon=3, min_samples = 1):
    ...
    disp_unfold = F.unfold(
        F.pad(disp, (patch_w // 2, patch_w // 2, patch_h // 2, patch_h // 2), mode='reflect'),
    ...
    disp_cluster = torch.mean(
        disp_unfold_clone.unsqueeze(1).repeat(1, patch_h * patch_w, 1, 1, 1)
        * mask_cluster, dim=2) * (patch_h * patch_w) / torch.sum(mask_cluster, dim=2).clamp(min=1)
    ...
    w_cluster[:, 0, ...] += 0.8 - 0.2 / (mask_cluster.sum(dim=1, keepdim=False) - 1 + 1e-30).clamp(min=1)
    ...
    w_cluster = w_cluster / (w_cluster.sum(dim=1, keepdim=True).clamp(min=1) + 1e-30)
