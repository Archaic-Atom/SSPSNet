...
try:
    ...
    from ._gen_gt_disb import adaptive_multi_modal_cross_entropy_loss
except ImportError:
    from _warp import Warp
    from Networks import build_gwc_volume
    from Networks import DispRegression
...
class Loss(object):
    ...
    def matching_loss(self, disp_list: list, disp_label: torch.Tensor,
        ...
        if udc:
            loss_2 = 0.5 * adaptive_multi_modal_cross_entropy_loss(
                disp_list[3], disp_label, mask_disp, args.disp_num) +\
                0.7 * adaptive_multi_modal_cross_entropy_loss(
                disp_list[4], disp_label, mask_disp, args.disp_num) +\
                adaptive_multi_modal_cross_entropy_loss(
                disp_list[5], disp_label, mask_disp, args.disp_num)
