...
at::Tensor broadcast_to(at::Tensor v, at::Tensor x) {
  if (x.ndimension() == 2) {
    ...
    for (int64_t i = 2; i < x.ndimension(); ++i) broadcast_size.push_back(1);

    return v.view(broadcast_size);
  }
}

at::Tensor BatchNorm_Forward_CPU(const at::Tensor input, const at::Tensor mean,
                                 const at::Tensor std, const at::Tensor gamma,
...
std::vector<at::Tensor> BatchNorm_Backward_CPU(
    const at::Tensor gradoutput, const at::Tensor input, const at::Tensor mean,
    const at::Tensor std, const at::Tensor gamma, const at::Tensor beta,
    bool train) {
  /* outputs*/
...
std::vector<at::Tensor> Sum_Square_Forward_CPU(const at::Tensor input) {
  /* outputs */
...
at::Tensor Sum_Square_Backward_CPU(const at::Tensor input,
