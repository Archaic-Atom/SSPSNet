...
template <typename DType, typename Acctype>
static __device__ __forceinline__ Float2<DType, Acctype> warpSum(
    Float2<DType, Acctype> value) {
...
__device__ T reduceD(Op op, int b, int i, int k, int D) {
  T sum = 0;
...
template <typename T, typename Op>
__device__ T reduceN(Op op, int b, int k, int d, int N) {
  T sum = 0;
...
template <typename T, typename Op>
__device__ T reduceK(Op op, int b, int i, int d, int K) {
  T sum = 0;
...
template <typename T, typename Op>
__device__ T reduceBN(Op op, int k, int d, int B, int N) {
  T sum = 0;
