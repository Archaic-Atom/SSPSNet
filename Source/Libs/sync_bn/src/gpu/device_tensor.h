...
struct DeviceTensor {
 public:
  ...
  inline __device__ __host__ DeviceTensor<DType, Dim - 1> select(
  ...
  inline __device__ __host__ DeviceTensor<DType, Dim - 1> operator[](
      const size_t x) const {
    assert(Dim > 1);
    int offset = x;
    for (int i = 1; i < Dim; ++i) {
  ...
  inline __device__ __host__ DType *data_ptr() const { return dptr_; }
...
struct DeviceTensor<DType, 1> {
  ...
  inline __device__ __host__ int numElements() const { return size_[0]; }
